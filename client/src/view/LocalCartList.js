...
const LocalCartList = ({ cartList, setCartReset, cartListUpdate }) => {
  ...
  const totalCount = () => {
    ...
    cartList.map((item) => (sumCount += item.price * item.count));
    return sumCount;
  };
  ...
  const selectTotalCount = () => {
    let sumCount = 0;
    cartList.map((item) => {
      if (item.isCheck == true) sumCount += item.price * item.count;
    });
    return sumCount;
  };
  ...
  const selectDeliveryFee = () => {
    const totalAmount = selectTotalCount();
    if (totalAmount > deliveryFeeFreeAmount) return 0;
    else return 5000;
  };
  ...
  return (
    <div>
      ...
      <hr></hr>
      <h1>
        총 상품 금액 : {totalCount()} 원 + 배송비 {allDeliveryFee()}원 = 총 주문
        금액 : {totalCount() + allDeliveryFee()} 원
      </h1>
      <h2>
        선택한 상품 금액 : {selectTotalCount()} 원 + 배송비{" "}
        {selectDeliveryFee()}원 = 총 주문 금액 :{" "}
        {selectTotalCount() + selectDeliveryFee()} 원
      </h2>
    </div>
  );
};
