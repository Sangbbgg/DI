...
const Shop = () => {
    ...
    const onClickBasket = (product) => {
        ...
        // 해당 상품의 데이터를 localstorage의 basket에 추가
                        ...
                        <div key={product.id} className={styles.proditem}>
                            <ul>
                                <Link to={`/shop/${product.id}`}>
                                    <img src={'https://onlyeco.co.kr/web/product/big/202011/5024dcdeb85c03cfa7467e0897d9781d.jpg'} className={styles.itemimg} alt="이미지"/>
                                </Link>
                                <li>{product.name}</li>
                                <li>{product.price}</li>
                                <button onClick={() => onClickBasket(product)}>장바구니 추가</button>
                            </ul>
                        </div>
                    ))}
                </div>
